...
import 'package:bucks_app/ui/widget/footer_widget.dart';
import 'package:bucks_app/ui/widget/loading_widget.dart';
import 'package:bucks_app/ui/widget/tabBar_details_widget.dart';
...
class _DetailsPreoductScreenState extends State<DetailsPreoductScreen> {
  ...
  void initState() {
    ...
    getIt<DetailsProductsProvider>().attributes['product_id'] =widget.ProdectId;
    super.initState();
  }
                    ...
                    children: [
                      ...
                      if (_tabClick == 0)
                        Container(
                          ...
                          child: Column(
                            children: [
                              buildContainerSilder(value.categoryProduct!,value.listVariation),
                            ],
                          ),
                        )
                      else if (_tabClick == 1)
  ...
  Container buildContainerSilder(CategoryProduct value, List<VariationModel> list) {
    log('type =>'+value.type!);
    log('type =>'+value.attributes![0].name!);
    return Container(
        ...
        children: [
          ...
          ListView.builder(
              ...
              itemBuilder: (context, index) {
                log('select =>${list[index].selected}');
                return list[index].selected ==true ?  Container(
                  height: 60,
                  child: Column(
                            ...
                            return GestureDetector(
                              onTap: (){
                                /*setState(() {
                                  ...
                                  log('index =>$i');
                                  if(i<= list.length && list[i].selected == false){
                                    list[i].selected =true;
                                    getIt<DetailsProductsProvider>().attributes['attributes']['${list[index].name}'] ='${list[index].options[ind]}';
                                    log('attributesattributes');
                                    print(getIt<DetailsProductsProvider>().attributes);
                              ...
                              child: Container(
                                  ...
                                  child: Text('${value.attributes![index].options![ind]}')),
                            );
                          },),
                      )
                    ],
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return Container(
                    child: Column(
                      children: [
                        Text('${value.attributes![index].name}'),
                        Container(
                          height: 30,
                          child: ListView.builder(
                          //  shrinkWrap: true,
                            scrollDirection: Axis.horizontal,
                            itemCount: value.attributes![index].options!.length,
                            itemBuilder: (context, ind) {
                              return GestureDetector(
                                onTap: (){
              ...
              if(i==0)
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.end,
                    children: [
                      Text(value.attributes![0].name!),
                      Expanded(
                        child: ListView.builder(
                          //shrinkWrap: true,
                          reverse:  true,
                          scrollDirection: Axis.horizontal,
                          itemCount: value.attributes![0].options!.length,
                          itemBuilder: (context, index) {
                          return GestureDetector(
                            onTap: (){
                              i = i+1;
                            ...
                            child: Container(
                                padding: EdgeInsets.all(8),
                                child: Text('${value.attributes![0].options![index]}')),
                          );
                        },),
                      )
                    ],
